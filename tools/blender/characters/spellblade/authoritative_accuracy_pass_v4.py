...
def _rebuild_pauldrons(
    ...
    for side, sign in (("L", -1.0), ("R", 1.0)):
        cx = 0.430 * sign
        # A dark wrapped shell gives side depth; the steel face sits on it like
        # the layered shield pauldron in the concept instead of reading as a ring.
        shell = _loft(
            f"Pauldron.{side}",
            (
        ...
        facet_points = tuple((x * sign, z) for x, z in raw)
        facet = _prism_xz(
...
def _bulk_limbs_and_refine_material_read(
    ...
    for obj in model.objects:
        ...
        if name.startswith(("PauldronTrim.", "PauldronLower.", "ShoulderBadge.", "PauldronTrimTop.")):
            _scale_about_center(obj, 0.86, 1.05, 1.01)
            ...
            _scale_about_center(obj, 0.84, 1.0, 0.84)
        elif name.startswith(("SwordGuard", "SwordPommel")):
            _scale_about_center(obj, 1.08, 1.08, 1.08)
        elif name == "SwordGem":
            _scale_about_center(obj, 1.22, 1.16, 1.22)
