...
7. **Stance and living holds** (`stance.py`):
   ...
   - **First person:** gets the same guard loop. Its bones rotate in Euler mode, so the loop is keyed on `rotation_euler` there (keying quaternions left the exported first-person Guard cut to 0.27 s).
8. **Sprint** (`sprint.py`): derives the Sprint action from Run. The cycle tightens from 21 to 17 frames, strides and the spell-arm pump swing 20–50 % further from the cycle's mean pose, and the torso leans in while the head comes up. The sword arm carries the blade back and low, clear of the legs. The pelvis bob grows 40 %, and every key is re-planted so the lower foot touches the ground where Run's did. The runtime plays Sprint while the Spellblade is in its sprint state (Shift, or the touch stick pushed fully forward).
9. **Slash follow-through** (`followthrough.py`): interpolating from a slash's follow-through key to its end pose swung the blade low across the front of the legs and below the floor. For each slash this adds one arm key inside the follow-through and lets the action's own curves interpolate it, so the blade's path bends around the legs. The search tries swinging the arm out, raising it forward, swinging it back, turning it outward and tipping the blade forward, over one or two key frames and increasing angles. It keeps the smallest correction that clears the body and floor without moving the blade tip faster than the original motion. The windup (start to contact) is checked the same way. The corrections are chosen by the search, so the key frames and angles can change from one build to the next. Contact frames, end frames and every other bone are unchanged.
