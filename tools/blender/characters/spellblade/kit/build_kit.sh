...
run() {
  ...
  print -r -- "$log" | grep -E "^(RIG2|HELMET3|KIT|BANNER|WEAR|FT |FOLLOWTHROUGH)" || true
}
# never copy a stale file from an earlier run
rm -f $WORK/helmet.blend $WORK/rig2.blend $WORK/kit.blend $WORK/kit_ft.blend $WORK/fp_kit.blend
...
run $WORK/kit.blend --python-exit-code 1 --python $KIT/followthrough.py -- $WORK/kit_ft.blend
